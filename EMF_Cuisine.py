...
def matrix_factorization(R, P, Q, K, W, E, steps, alpha, beta, lamda):
    Q = Q.T
        ...
        for i in range(len(R)):
            for j in range(len(R[i])):
                if R[i][j] > 0:
                    ...
                    for k in range(K):
                        P[i][k] = P[i][k] + alpha * (2 * eij * Q[k][j] - beta * P[i][k] - lamda * (P[i][k]-Q[k][j])*W[i][j])
                        Q[k][j] = Q[k][j] + alpha * (2 * eij * P[i][k] - beta * Q[k][j] + lamda * (P[i][k]-Q[k][j])*W[i][j])
        eR = numpy.dot(P, Q)
        e = 0
        count = 0
        for i in range(len(R)):
            for j in range(len(R[i])):
                if R[i][j] > 0:
                   ...
                   # e1 = e
                    for k in range(K):
                        e = e + (beta / 2) * (pow(P[i][k], 2) + pow(Q[k][j], 2)) + (lamda/2)*(pow((P[i][k]-Q[k][j]),2)*W[i][j])
        print step
...
#    pickle.dump(P, open("P60k_W_last10%fortest", "wb"))
#    pickle.dump(Q, open("Q60k_W_last10%fortest", "wb"))
    pickle.dump(P, open("../Experiments/EMF/P/"+str(E)+"_P60k_k"+str(K)+"_alpha"+str(alpha)+"_beta"+str(beta)+"_lambda"+str(lamda)+"_first_90train", "wb"))
    pickle.dump(Q, open("../Experiments/EMF/Q/"+str(E)+"_Q60k_k"+str(K)+"_alpha"+str(alpha)+"_beta"+str(beta)+"_lambda"+str(lamda)+"_first_90train", "wb"))
    #pickle.dump(P, open("../Experiments/EMF_test25/P/" + str(E) + "_P60k_k" + str(K) + "_alpha" + str(alpha) + "_beta" + str(beta) + "_lambda" + str(lamda) + "_first_90train", "wb"))
    #pickle.dump(Q, open("../Experiments/EMF_test25/Q/" + str(E) + "_Q60k_k" + str(K) + "_alpha" + str(alpha) + "_beta" + str(beta) + "_lambda" + str(lamda) + "_first_90train", "wb"))
...
if __name__ == "__main__":
    ...
    data = pandas.read_csv("../Restaurant Data with Consumer Ratings/rating_final1.csv", sep=',',
                           names="userID,placeID,food_rating".split(","))
    alldata = pandas.read_csv("../Restaurant Data with Consumer Ratings/Merged1.csv", sep=',',
                              names="userID,uniqueUserID,placeID,uniquePlaceID,rating,food_rating,service_rating,cuisine,cuisineID".split(
                                  ","))
    cuisinedata = pandas.read_csv("../Restaurant Data with Consumer Ratings/chefmozcuisine.csv", sep=',',
                                  names="placeID,Rcuisine".split(","))
    ...
    for K in range(5, 51, 5):
        ...
        for E in range(1, 11, 1):
            ...
            N = len(train_data_matrix)
            M = len(train_data_matrix[0])
            # K = 10
            ...
            beta = 0.001
            lamda = 0.005
            nP, nQ = matrix_factorization(train_data_matrix, P, Q, K, W, E, steps, alpha, beta, lamda)
            nR = numpy.dot(nP, nQ)
            """
    
            nP = pickle.load(open("../Experiments/MF/P/"+str(E)+"_P60k_k"+str(K)+"_alpha0.01_beta0.1_first_90train", "rb"))
            nQ = pickle.load(open("../Experiments/MF/Q/"+str(E)+"_Q60k_k"+str(K)+"_alpha0.01_beta0.1_first_90train", "rb"))
    
            nR = numpy.dot(nP, nQ)
            """

            rmse1 = rmse(nR, train_data_matrix)

            # print ('Cross-Validation Step ' + str(counter) + ' for test' + ":\t" + str(rmse1))
            rmse2 = rmse(nR, test_data_matrix)

            # print(E, rmse1, rmsetrain, rmsetrainmean)
            # print(E, rmse2, rmsetest, rmsetestmean)
...
"""
    ...
    evaluation.to_csv("../Experiments/EMF/Results/EMF_Evaluation_" + str(K) + "_alpha" + str(alpha) + "_beta" + str(
        beta) + "_first90train.csv", sep='\n', header=False, float_format='%.2f', index=False, )
