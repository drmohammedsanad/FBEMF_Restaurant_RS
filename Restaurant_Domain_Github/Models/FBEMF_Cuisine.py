...
#    pickle.dump(Q, open("Q60k_W_last10%fortest", "wb"))
    pickle.dump(P, open("../Experiments/FBEMF/P/"+str(E)+"_P60k_k"+str(K)+"_alpha"+str(alpha)+"_beta"+str(beta)+"_lambda"+str(lamda)+"_first_90train", "wb"))
    pickle.dump(Q, open("../Experiments/FBEMF/Q/"+str(E)+"_Q60k_k"+str(K)+"_alpha"+str(alpha)+"_beta"+str(beta)+"_lambda"+str(lamda)+"_first_90train", "wb"))
    #pickle.dump(P, open("../Experiments/FBEMF_test25/P/" + str(E) + "_P60k_k" + str(K) + "_alpha" + str(alpha) + "_beta" + str(beta) + "_lambda" + str(lamda) + "_first_90train", "wb"))
    #pickle.dump(Q, open("../Experiments/FBEMF_test25/Q/" + str(E) + "_Q60k_k" + str(K) + "_alpha" + str(alpha) + "_beta" + str(beta) + "_lambda" + str(lamda) + "_first_90train", "wb"))
...
if __name__ == "__main__":
    ...
    #data = pandas.read_csv("Restaurant Data with Consumer Ratings/rating_final.csv",sep=',',names="userID,uniqueUserIDs,placeID,uniquePlaceIDs,rating,food_rating,service_rating".split(","))

    data = pandas.read_csv("../Restaurant Data with Consumer Ratings/rating_final1.csv",sep=',',names="userID,placeID,food_rating".split(","))
    ...
    for K in range(5, 51, 5):
        ...
        for E in range(1, 11, 1):
            ...
            beta = 0.001
            lamda = 0.0001
            nP, nQ = matrix_factorization(train_data_matrix, P, Q, K, SW_norm, E, steps, alpha, beta, lamda)
            nR = numpy.dot(nP, nQ)
            """
...
"""
    ...
    evaluation.to_csv("../Experiments/FBEMF/Results/FBEMF_Evaluation_" + str(K) + "_alpha" + str(alpha) + "_beta" + str(
